...
func CreateSchedule(destination string, hotel models.Place, startDate string, endDate string) string{
  ...
  duration := t2.Sub(t1)
	...
	for i := 0; i < days; i++ {
		...
		res += fmt.Sprintf("At day %d, you can visit:\n", i + 1)
		for curHour < RETURN_TIME {
			res += attractionStr(i + 1, attractions[curPlaceIdx].Name, curHour)
		...
		res += restaurantStr(i + 1, restaurants[i % len(restaurants)].Name, curHour)
	}
